...
"""Semantic embedding fuzzy-match provider.

Thin SGLang-side wrapper around the ``semblend`` pip package's adapter.
...
class SemanticEmbeddingProvider(FuzzyMatchProvider):
    """Embedding-based semantic fuzzy match using SemBlend.
    ...
    def _decode(self, request, token_ids: List[int]) -> str:
        ...
        intentionally accept best-effort here. When no tokenizer is reachable
        the adapter still embeds via SemBlend's tokenizer bridge so this is
        a fast-path optimization, not a hard requirement.
        """
        tokenizer = None
